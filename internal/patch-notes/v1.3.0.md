...
Two entries merged to `master` since `v1.2.0`, both part of the same
offline-first push driven by real Discord feedback.

## Known issues in this release — Linux/Steam Deck is unstable

**Windows is the recommended, verified platform for v1.3.0.** Linux is
shipped as source-plausible only, not hardware-verified:

- **Steam Deck / SteamOS: the launcher does not currently work at all** —
  the webview opens blank (`EGL_BAD_PARAMETER`, a bundled `libwayland-*`
  conflict with SteamOS's own Mesa/EGL stack). A build-pipeline fix
  exists but has never been run against real Steam Deck hardware — see
  `BUGS.md`'s 2026-09-24 entry.
- **Linux icon extraction** (the new `Warframe-Exporter-CLI.AppImage`
  path, see below) is similarly unverified on any real Linux machine —
  reasoned through defensively (chmod, FUSE fallback) but never actually
  run there. Not a crash risk if wrong — it just leaves placeholder icons
  instead of real ones.
- General Linux desktop (non-Steam-Deck) support is unconfirmed either
  way — it may well work, it just hasn't been tested.

Shipping anyway rather than holding this release hostage to hardware
neither of us has on hand — see `README.md`/`DEVLOG.md` for the
user-facing version of this same caveat.
